...
namespace Zhiyi\PlusGroup\Handlers;

use Illuminate\Console\Command;
use Zhiyi\PlusGroup\Support\Path;
...
class DevPackageHandler extends \Zhiyi\Plus\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.plus-group.migrations')
        );
    ...
    {
        $path = $this->app->make('path.plus-group.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-group.resources').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-group.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
