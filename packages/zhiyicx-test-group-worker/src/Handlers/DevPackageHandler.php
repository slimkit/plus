...
namespace Zhiyi\Plus\Packages\TestGroupWorker\Handlers;
...
class DevPackageHandler extends \Zhiyi\Plus\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.test-group-worker.migrations')
        );
    ...
    {
        $path = $this->app->make('path.test-group-worker.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.test-group-worker.resources').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.test-group-worker.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
