...
@section('content')
    ...
    <form class="form-horizontal" method="post" action="{{ route('music:store') }}">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <input type="text" name="title" class="form-control" value="" placeholder="歌曲名称">
            </div>
        </div>
        <div class="form-group">
        	<label for="" class="col-sm-2 control-label">上传歌曲</label>
        	<div class="col-sm-6">
        		<input type="file" name="file" class="projectfile" id="projectfile" accept="audio/*" data-preview-file-type="audio" value="" />
        ...
        <div class="form-group">
        	...
        	<div class="col-sm-6">
        		<input type="number" name="sort" value="0" class="form-control" placeholder="歌曲权重, 越大越靠前" />
        	</div>
        </div>
        <div class="form-group">
        	...
        	<div class="btn-group col-sm-6" data-toggle="buttons">
			  	...
			  	<label class="btn btn-primary">
			    	<input type="checkbox" value="download" class="paid_node pay" name="paid_node[]" autocomplete="off"> 收费
			  	</label>
			</div>
        </div>
		<div class="form-group">
			...
			<div class="col-sm-6">
				<input type="number" id="download-pay" name="download" disabled="" class="form-control" placeholder="金额单位: 元">
			</div>
		</div>
		<div class="form-group">
			<label for="listen" class="col-sm-2 control-label">歌曲长度</label>
			<div class="col-sm-6">
				<input type="number" id="last_time" name="last_time" class="form-control" placeholder="单位: 秒">
			</div>
		</div>
        <div class="form-group">
        	...
        	<div class="btn-group col-sm-6" data-toggle="buttons">
        		@foreach ($specials as $special)
				  	<label class="btn btn-primary">
				    	<input type="checkbox" name="special[]" value="{{ $special->id }}" autocomplete="off"> {{ $special->title }}
				  	</label>
			  	@endforeach
			</div>
        </div>
        <div class="form-group">
        	...
        	<div class="btn-group col-sm-6" data-toggle="buttons">
        		...
        		@foreach ($singers as $singer)
				  	<label class="btn btn-primary">
				    	<input type="radio" value="{{ $singer->id }}" name="singer"autocomplete="off"> {{ $singer->name }}
				  	</label>
			  	@endforeach
			</div>
        </div>
        <div class="form-group">
        	...
        	<div class="col-sm-6">
        		<textarea name="lyric" class="form-control" rows="5" placeholder="请输入歌词"></textarea>
        	</div>
        </div>
        <div class="form-group">
