...
class DevPackageHandler extends \Zhiyi\Plus\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.plus-appversion.migrations')
        );
    ...
    public function makeSeederHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-appversion.resource').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-appversion.resource').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
