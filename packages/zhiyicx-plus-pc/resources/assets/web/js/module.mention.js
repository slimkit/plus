var mention = {
    ...
    handleString: function (text) {
        ...
        var text2 = '@' + text + ' '
        var text3 = this.objString.substr(this.cursorPosition, this.getLength(this.objString) - this.cursorPosition)
        ...
        if (this.indexString !== undefined && this.indexString.length > 1) {
            ...
            if (list !== undefined && list.length > 0) {
                dom = this.indexString.length > 1
                    ? '<li class="list-title">选择好友或直接输入</li>'
                    : '<li class="list-title">选择用户名或轻敲空格完成输入</li>'
                for (var i = 0; i < list.length; i++) dom += '<li class="list-content">' + list[i] + '</li>'
            } else {
