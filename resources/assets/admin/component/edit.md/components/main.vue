...
<script>
  // 引入依赖库
  import marked from '../utils/ts_marked'
  ...
  renderer.ts_img = (href, title, text) => {
    var out = '<img src="' + window.NEWS.files + '/' + href + '" alt="' + text + '"';
    if (title) {
      out += ' title="' + title + '"';
    }
  ...
  export default {
    ...
    data() {
      return {
        ...
        vmdEditor: null,
        vmdPreview: null,
        vmdInput: '# Type anything you want to edit...',
        lang: 'zh',
        isPreview: true,
        isSanitize: false
      }
